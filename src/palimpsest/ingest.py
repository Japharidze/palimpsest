...
def refresh_companies(client, storage, conn) -> int:
    ...
    key = (
        f"raw/reference/company_tickers/{datetime.now(tz=UTC).date().isoformat()}.json"
    )
...
def sync_filings(client: EdgarClient, storage, conn, cik) -> int:
    ...
    list_of_recents = [
        dict(zip(recent_lists.keys(), values)) for values in zip(*recent_lists.values())
    ]
    rows = [
        _parse_submission(f, cik)
        for f in list_of_recents
        if f["primaryDocument"] and f["form"] in TRACKED_FORMS
    ]
