...
**Transform.** dbt builds normalized marts from the raw tables. dbt tests carry the data-quality checks — unexpected nulls, tag drift, restatement detection.

**Rules.** Plain Python over the marts. Going-concern language, auditor changes, late filings, restatements, metric deterioration. Deterministic, testable, and the input to triage.
