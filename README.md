...
FastAPI serves the agent and the warehouse; a React frontend sits over it. Everything happens in one transcript — questions, the tool calls the agent made, answers, and company cards produced by clicking the watchlist — so the conversation is the whole record of a session.

Citations are clickable. A citation naming a section opens that section with the quoted passage highlighted and scrolled into view. A citation naming only a filing is a figure from XBRL, and opens the tagged facts that filing reported: the tag the company used, the period, and the value as filed.
...
- **Foreign private issuers.** They file 20-F and no 10-Q, so they appear in the yearly metrics and not the quarterly ones. Section detection on 20-F filings is also inconsistent across years, so those filings are excluded from language diffing.
...
- **Table-heavy sections.** Financial statements change in every figure every quarter, so they are indexed and searchable but not diffed.
- **Small local models.** Models below roughly 8B parameters do not reliably compose multiple tools or follow citation instructions. The agent is intended to run against a hosted model; local models are for development.
- **Metric provenance.** A figure's source filing is taken from the XBRL facts, which can name a filing older than the window ingested for that company. The citation is then unresolvable even though the number is correct.
- **Absence answers.** When the honest answer is that a filing does not contain something, there is nothing to cite, and the citation check reports that as a problem.
